...
def open_win():
    ...
    apt = Tk()
    apt.title("Interface")
    Label(apt, text="ADANI GENMART").grid(row=0, column=0, columnspan=3)
    ...
    Button(apt, text='VIP Customer', width=25, bg='green', fg='white', command=val_cus).grid(row=4, column=0)
...
def ren():
    ...
    def onmousewheel():
        lb1.ywiew = ('scroll', events.delta, 'units')
        lb2.ywiew = ('scroll', events.delta, 'units')
        return 'break'
...
def ref():
    ...
    def onmousewheel():
        lb1.ywiew = ('scroll', events.delta, 'units')
        lb2.ywiew = ('scroll', events.delta, 'units')
        lb3.ywiew = ('scroll', events.delta, 'units')
        lb4.ywiew = ('scroll', events.delta, 'units')
        lb5.ywiew = ('scroll', events.delta, 'units')
        lb6.ywiew = ('scroll', events.delta, 'units')
...
def exp_date():
    ...
    if flags == 'apt1':
        Button(exp, text='Main Menu', bg='green', fg='white').grid(row=5, column=2)
    else:
...
def search():
    ...
    if flags == 'apt1':
        Button(st, width=15, text='Main Menu', bg='green', fg='white').grid(row=6, column=2)
    else:
...
def again():
    ...
    root.title('Login')
    Label(root, text='ADANI GENMART').grid(row=0, column=0, columnspan=5)
    ...
    pwd.config(show='*')
    Button(root, width=6, bg='blue', fg='white', text='Enter', command=check).grid(row=5, column=1)
    root.mainloop()
